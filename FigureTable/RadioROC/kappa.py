...
def heatmap(data, row_labels, col_labels, ax=None,
    ...
    **kwargs
    ...
    cbar.ax.set_ylabel(cbarlabel, rotation=-90, va="bottom", fontsize=18, fontweight='black')
...
def agreement(mode):
    agree = np.ones((7, 7)) * 100
    average = []
    for i in range(7):
        for j in range(i):
            resA = get_diags('kappa/n{}.csv'.format(i + 1), mode)
            resB = get_diags('kappa/n{}.csv'.format(j + 1), mode)
    ...
    im, cbar = heatmap(agree,
                       ['radio. {}'.format(i) for i in range(1, 8)],
