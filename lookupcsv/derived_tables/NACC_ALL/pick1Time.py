...
with open('mri_diag_table_6months.csv', 'r') as csvfile:
    ...
    for row in reader:
        readMap[row['ID']] = (row['zipname'], row['NC'], row['MCI'], row['DE'], row['COG'],
                              row['AD'], row['PD'], row['FTD'], row['VD'], row['DLB'], row['LBD'], row['PDD'], row['Other'])

for idx in [10, 9, 8, 7, 6, 5, 3, 2, 1, 12]:
    for id in readMap:
...
with open('unique_mri_diag_table_6months.csv', 'w') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=['zipname', 'ID', 'NC', 'MCI', 'DE', 'COG', 'AD', 'PD', 'FTD', 'VD', 'DLB', 'LBD', 'PDD', 'Other'])
    writer.writeheader()
    case = {}
    for id in content:
        case['zipname'] = content[id][0]
        case['ID'] = id
