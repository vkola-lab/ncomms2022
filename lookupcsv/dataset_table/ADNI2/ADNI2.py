...
class TableData:
    def __init__(self):
        self.datasetName = 'ADNI2'
        self.imageDir = '/data/datasets/ADNI2/'
    ...
    def addColumns_demograph(self):
        ...
        for row in targetTable:
            if row['Phase'] == 'ADNI2' and row['VISCODE2'] == 'sc' and row['RID'] in self.content:
    ...
    def addColumns_diagnosis(self):
        ...
        for row in targetTable:
            if row['Phase'] == 'ADNI2' and row['VISCODE2'] == 'sc' and row['RID'] in self.content:
                if row['DXCHANGE'] in ['1', '7', '9']:  # NL healthy control
                ...
                elif row['DXCHANGE'] in ['2', '4', '8']:  # MCI patient
                    for var in ['NC', 'DE', 'AD', 'FTD', 'VD', 'DLB', 'PDD', 'OTHER']:
                        self.content[row['RID']][var] = 0
                    self.content[row['RID']]['MCI'] = 1
                    self.content[row['RID']]['COG'] = 1
                elif row['DXCHANGE'] in ['3', '5', '6']:  # Dementia patient
                    self.content[row['RID']]['NC'] = 0
                    self.content[row['RID']]['MCI'] = 0
                    self.content[row['RID']]['DE'] = 1
                    self.content[row['RID']]['COG'] = 2
                    self.content[row['RID']]['ADD'] = 0
                    if row['DXDDUE'] == '1':
                        self.content[row['RID']]['AD'] = 1
                        self.content[row['RID']]['ADD'] = 1
                    elif row['DXDDUE'] == '2':
                        if row['DXODES'] == '1': self.content[row['RID']]['FTD'] = 1
                        if row['DXODES'] == '12': self.content[row['RID']]['FTD'] = 1
                        if row['DXODES'] == '2': self.content[row['RID']]['PD'] = 1
                        if row['DXODES'] == '2': self.content[row['RID']]['PDD'] = 1
                        if row['DXODES'] == '9': self.content[row['RID']]['VD'] = 1
    ...
    def addColumns_cdr(self):
        # variables = ['cdr', 'cdr_memory', 'cdr_orient', 'cdr_judge', 'cdr_commu', 'cdr_home', 'cdr_care']
        # old_variables = ['CDGLOBAL','CDMEMORY','CDORIENT','CDJUDGE','CDCOMMUN','CDHOME','CDCARE']
        # self.columnNames.extend(variables)
    ...
    def addColumns_moca(self):
        ...
        targetTable = self.readcsv(targetTable)
        for row in targetTable:
            if row['Phase'] == 'ADNI2' and row['VISCODE'] == 'v03' and row['RID'] in self.content:
                for old_var, new_var in zip(old_variables, variables):
                    self.content[row['RID']][new_var] = row[old_var]
    ...
    def addColumns_adas(self):
        ...
        targetTable = self.readcsv(targetTable)
        for row in targetTable:
            if row['Phase'] == 'ADNI2' and row['VISCODE'] == 'bl' and row['RID'] in self.content:
                for old_var, new_var in zip(old_variables, variables):
                    self.content[row['RID']][new_var] = row[old_var]
    ...
    def addColumns_logicalmemory(self):
        ...
        targetTable = self.readcsv(targetTable)
        for row in targetTable:
            if row['Phase']=='ADNI2' and row['VISCODE2'] in ['sc', 'bl'] and row['RID'] in self.content:
    ...
    def addColumns_NPIQ(self):
        ...
        targetTable = self.readcsv(targetTable)
        for row in targetTable:
            if row['Phase'] == 'ADNI2' and row['VISCODE2'] == 'bl' and row['RID'] in self.content:
    ...
    def addColumns_FHQ(self):
        ...
        targetTable = self.readcsv(targetTable)
        for row in targetTable:
            if row['PHASE'] == 'ADNI2' and row['VISCODE'] == 'v01' and row['RID'] in self.content:
    ...
    def addColumns_dep(self):
        ...
        targetTable = self.readcsv(targetTable)
        for row in targetTable:
            if row['Phase'] == 'ADNI2' and row['VISCODE2'] in ['bl', 'sc'] and row['RID'] in self.content:
